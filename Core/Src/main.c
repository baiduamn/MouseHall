...
composite_hid_report_t hid_report = {0};
...
void Mouse_XY_Updata(void)
{
	...
	X_Speed = (int8_t)(motion_burst_data[2] + (motion_burst_data[3] << 8));
	Y_Speed = (int8_t)(motion_burst_data[4] + (motion_burst_data[5] << 8));
	
	myMouse_update(&hid_report);
}
